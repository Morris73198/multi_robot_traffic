...
class PathFollower(Node):
    ...
    #         file_path = os.path.join(current_dir, 'scan_data.json')
    #         temp={"ranges":list(self.ranges),
    ...
    def scan_callback(self, msg, robot_name):
        ...
        #for 琮閔資料蒐集
        """
        處理來自指定機器人 LaserScan 數據的回調函數
        :param msg: LaserScan 消息
        :param robot_name: 機器人的名稱 (例如 robot1, robot2)
        """
        try:
            # 記錄來自哪台機器人的數據
            self.get_logger().info(f'Received LaserScan from {robot_name}')
            ...
            self.get_logger().info(f'{robot_name} min distance: {min_distance:.2f} meters')

            # 保存數據到檔案
            current_dir = os.getcwd()  # 取得當前目錄
            file_path = os.path.join(current_dir, f'{robot_name}_scan_data.json')
            data_to_save = {
            ...
            self.get_logger().info(f'Scan data saved to {file_path}')

        except Exception as e:
            # 處理任何例外
            self.get_logger().error(f'Error in scan_callback for {robot_name}: {str(e)}')
